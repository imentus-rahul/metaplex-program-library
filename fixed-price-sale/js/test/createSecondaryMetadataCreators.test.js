"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var tape_1 = require("tape");
var amman_1 = require("@metaplex-foundation/amman");
var src_1 = require("../src");
var utils_1 = require("./utils");
var actions_1 = require("./actions");
var transactions_1 = require("./transactions");
(0, utils_1.killStuckProcess)();
(0, tape_1)('create-secondary-metadata-creators: success', function (t) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, payer, connection, transactionHandler, store, metadata, creator, _b, secondaryMetadataCreators, createSecondaryMetadataCreatorsTx, createSecondaryMetadataCreatorsRes;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0: return [4 /*yield*/, (0, actions_1.createPrerequisites)()];
            case 1:
                _a = _c.sent(), payer = _a.payer, connection = _a.connection, transactionHandler = _a.transactionHandler;
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 17 ~ test ~ transactionHandler", transactionHandler["payer"].publicKey.toBase58());
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 17 ~ test ~ connection", connection["_rpcEndpoint"]);
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 17 ~ test ~ payer", payer.publicKey.toBase58());
                return [4 /*yield*/, (0, actions_1.createStore)({
                        test: t,
                        transactionHandler: transactionHandler,
                        payer: payer,
                        connection: connection,
                        params: {
                            name: 'Store',
                            description: 'Description',
                        },
                    })];
            case 2:
                store = _c.sent();
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 31 ~ test ~ store", store.publicKey.toBase58());
                return [4 /*yield*/, (0, actions_1.initSellingResource)({
                        test: t,
                        transactionHandler: transactionHandler,
                        payer: payer,
                        connection: connection,
                        store: store.publicKey,
                        maxSupply: 100,
                    })];
            case 3:
                metadata = (_c.sent()).metadata;
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 34 ~ test ~ metadata - initSellingResource", metadata.toBase58());
                creator = src_1.CreatorAccountData.fromArgs({
                    address: payer.publicKey,
                    share: 100,
                    verified: false,
                });
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 48 ~ test ~ creator", creator.address.toBase58());
                return [4 /*yield*/, (0, transactions_1.createSecondaryMetadataCreators)({
                        test: t,
                        transactionHandler: transactionHandler,
                        payer: payer,
                        connection: connection,
                        metadata: metadata,
                        creators: [creator],
                    })];
            case 4:
                _b = _c.sent(), secondaryMetadataCreators = _b.secondaryMetadataCreators, createSecondaryMetadataCreatorsTx = _b.createSecondaryMetadataCreatorsTx;
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 51 ~ test ~ createSecondaryMetadataCreatorsTx", createSecondaryMetadataCreatorsTx);
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 51 ~ test ~ secondaryMetadataCreators", secondaryMetadataCreators.toBase58());
                return [4 /*yield*/, transactionHandler.sendAndConfirmTransaction(createSecondaryMetadataCreatorsTx, [payer], amman_1.defaultSendOptions)];
            case 5:
                createSecondaryMetadataCreatorsRes = _c.sent();
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 67 ~ test ~ createSecondaryMetadataCreatorsRes", createSecondaryMetadataCreatorsRes.txSignature);
                (0, utils_1.logDebug)("secondary-metadata-creators: ".concat(secondaryMetadataCreators.toBase58()));
                (0, amman_1.assertConfirmedTransaction)(t, createSecondaryMetadataCreatorsRes.txConfirmed);
                return [2 /*return*/];
        }
    });
}); });
(0, tape_1)('create-secondary-metadata-creators: empty creators', function (t) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, payer, connection, transactionHandler, store, metadata, creators, createSecondaryMetadataCreatorsTx, tx, err_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0: return [4 /*yield*/, (0, actions_1.createPrerequisites)()];
            case 1:
                _a = _b.sent(), payer = _a.payer, connection = _a.connection, transactionHandler = _a.transactionHandler;
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 75 ~ test ~ transactionHandler", transactionHandler["payer"].publicKey.toBase58());
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 75 ~ test ~ connection", connection["_rpcEndpoint"]);
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 75 ~ test ~ payer", payer.publicKey.toBase58());
                return [4 /*yield*/, (0, actions_1.createStore)({
                        test: t,
                        transactionHandler: transactionHandler,
                        payer: payer,
                        connection: connection,
                        params: {
                            name: 'Store',
                            description: 'Description',
                        },
                    })];
            case 2:
                store = _b.sent();
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 89 ~ test ~ store", store.publicKey.toBase58());
                return [4 /*yield*/, (0, actions_1.initSellingResource)({
                        test: t,
                        transactionHandler: transactionHandler,
                        payer: payer,
                        connection: connection,
                        store: store.publicKey,
                        maxSupply: 100,
                    })];
            case 3:
                metadata = (_b.sent()).metadata;
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 92 ~ test ~ metadata", metadata.toBase58());
                creators = [];
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 102 ~ test ~ creators", creators);
                return [4 /*yield*/, (0, transactions_1.createSecondaryMetadataCreators)({
                        test: t,
                        transactionHandler: transactionHandler,
                        payer: payer,
                        connection: connection,
                        metadata: metadata,
                        creators: creators,
                    })];
            case 4:
                createSecondaryMetadataCreatorsTx = (_b.sent()).createSecondaryMetadataCreatorsTx;
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 104 ~ test ~ createSecondaryMetadataCreatorsTx", createSecondaryMetadataCreatorsTx);
                console.log("Expecting Next Line will throw an error, Tx ID will not be printed, instead catch block will be executed");
                _b.label = 5;
            case 5:
                _b.trys.push([5, 7, , 8]);
                return [4 /*yield*/, transactionHandler.sendAndConfirmTransaction(createSecondaryMetadataCreatorsTx, [payer], amman_1.defaultSendOptions)];
            case 6:
                tx = _b.sent();
                console.log("🚀 ~ file: createSecondaryMetadataCreators.test.ts ~ line 121 ~ test ~ tx - sendAndConfirmTransaction: ", tx.txSignature);
                t.fail('expected transaction to fail');
                return [3 /*break*/, 8];
            case 7:
                err_1 = _b.sent();
                (0, amman_1.assertError)(t, err_1, [/custom program error/i, /0x1791/i]);
                return [3 /*break*/, 8];
            case 8: return [2 /*return*/];
        }
    });
}); });
